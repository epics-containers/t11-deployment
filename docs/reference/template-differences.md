...
| OPI exposure and port selection | t11-epics-opis no longer forks the chart: it depends directly on the upstream [`epics-opis` chart at 5.10.0](https://github.com/epics-containers/ec-helm-charts/releases/tag/5.10.0), with the local `templates/`, `config/nginx.conf` and Chart.yaml override removed; `values.yaml` sets only `service.type: ClusterIP` under the `epics-opis:` key. Upstream 5.10.0 now ships the Service type/port/external-IP/LoadBalancer/annotations configurability and the port-443 removal that t11 previously carried as a local patch. | **Done:** upstreamed in ec-helm-charts 5.10.0 (epics-opis); t11-services consumes it as a plain OCI dependency, no fork or template-side work remains. |
